...
def gradientDescent(x, y, alpha):
    ...
    while not np.all( np.absolute(gradient) <= 1e-9 ):
...
def ExpGradDescent():
    ...
    x  = np.hstack( (x0, x1) )
    y  = np.array( [3, 4, 5, 5, 2, 4, 7, 8, 11, 8, 12, 11,
                    13, 13, 16, 17, 18, 17, 19, 21]).reshape( m, 1 )
...
def gaussPivot(  a, b, tol= 1.0e-9 ):
    ...
    for k in range( 0, n-1 ):
        ...
        if abs( a[p, k] ) < tol: print( 'matrix is sigular' )
        if p != k:
            swapRows( b, k, p )
            swapRows( s, k, p )
            swapRows( a, k, p )
...
def ExpPlotFit():
    xData = np.array( [-0.04, 0.93, 1.95, 2.90, 3.83, 5.0, 5.98, 7.05, 8.21, \
                       9.08, 10.09 ])
    yData = np.array( [-8.66,-6.44,-4.36,-3.27,-0.88,0.87, 3.31, 4.63, 6.19, \
                   7.4, 8.85] )
    while True:
       try:
