#https://www.pytorch123.com/FifthSection/Chatbot/
#聊天机器人教程
...
corpus = os.path.join(rootDir, corpus_name)
print( corpus )
...
def unicodeToAscii(s):                      # 将Unicode字符串转换为纯ASCII，多亏了https://stackoverflow.com/a/518232/2809427
    return ''.join(c for c in unicodedata.normalize('NFD', s) if unicodedata.category(c) != 'Mn')
def readVocs(datafile, corpus_name):                                           # 初始化Voc对象 和 格式化pairs对话存放到list中
    print("Reading lines...")
...
def filterPairs(pairs):                                                                 # 过滤满足条件的 pairs 对话
    return [pair for pair in pairs if filterPair(pair)]
def loadPrepareData(corpus, corpus_name, datafile, save_dir):               # 使用上面定义的函数，返回一个填充的voc对象和对列表
...
def inputVar(l, voc):                                 # 返回填充前（加入结束index EOS_token做标记）的长度 和 填充后的输入序列张量
...
def batch2TrainData(voc, pair_batch):                                                           # 返回给定batch对的所有项目
...
class EncoderRNN(nn.Module):
    ...
    def forward(self, input_seq, input_lengths, hidden=None):
        ...
        embedded = self.embedding(input_seq)                                                        # 将单词索引转换为词向量

        packed = nn.utils.rnn.pack_padded_sequence(embedded, input_lengths)                    # 为RNN模块打包填充batch序列

        outputs, hidden = self.gru(packed, hidden)                                                          # 正向通过GRU

        outputs, _ = nn.utils.rnn.pad_packed_sequence(outputs)                                                  # 打开填充

        outputs = outputs[:, :, :self.hidden_size] + outputs[:, : ,self.hidden_size:]                    # 总和双向GRU输出

        return outputs, hidden                                                                      # 返回输出和最终隐藏状态
#4.3 解码器
class Attn(torch.nn.Module):                                                                    # Luong的attention layer
    ...
    def forward(self, hidden, encoder_outputs):
        if self.method == 'general':                                                        # 根据给定的方法计算注意力（能量）
...
def train(input_variable, lengths, target_variable, mask, max_target_len, encoder, decoder, embedding,
    ...
    encoder_optimizer.zero_grad()                                                                              # 零化梯度
    decoder_optimizer.zero_grad()

    input_variable = input_variable.to(device)                                                              # 设置设备选项
    ...
    decoder_input = torch.LongTensor([[SOS_token for _ in range(batch_size)]])  # 创建初始解码器输入（从每个句子的SOS令牌开始）
    ...
    use_teacher_forcing = \
        True if random.random() < teacher_forcing_ratio else False               # 确定我们是否此次迭代使用`teacher forcing`

    if use_teacher_forcing:                                                                # 通过解码器一次一步地转发一批序列
...
#solve: https://blog.csdn.net/weixin_43711726/article/details/112246264
