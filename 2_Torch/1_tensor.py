...
print(" create tensor with SP content: torch.tensor([5.5, 3]) \n  ", x)
x = x.new_ones(5, 3, dtype= torch.double)
print(" new_ones(5, 3) : \n  ", x)
x = torch.randn_like(x, dtype= torch.float)
print(" create tensor base on a tensor: \n  ", x)
print(" 获取张量的维度信息： \n  ", x.size())
...
y = torch.randn(5, 3)
print( " tentsor y : \n  ", y )
print( " tensor x add tensor y: x + y \n  ", x + y )
print( " torch.add(x, y) : \n  ", torch.add(x, y) )
...
y.add_( x )
print( " change content with _y.add_(x)==> y= : \n  ", y )
